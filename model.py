...
class ConditionalModel(nn.Module):
    def __init__(self, config, guidance=True):
        ...
        self.norm = nn.GroupNorm(1, feature_dim)  # GroupNorm(1) = InstanceNorm, works with batch_size=1

        self.encoder_x_l = ResNetEncoder(arch=arch, feature_dim=feature_dim, config=config, local=True)
        self.norm_l = nn.GroupNorm(1, feature_dim)  # GroupNorm(1) = InstanceNorm, works with batch_size=1
        
        if self.guidance:
            self.lin1 = ConditionalConv2d(y_dim * 2, feature_dim, n_steps)
        else:
            self.lin1 = ConditionalConv2d(y_dim, feature_dim, n_steps)
        self.unetnorm1 = nn.GroupNorm(1, feature_dim)  # GroupNorm(1) = InstanceNorm, works with batch_size=1
        self.lin2 = ConditionalConv2d(feature_dim, feature_dim, n_steps)
        self.unetnorm2 = nn.GroupNorm(1, feature_dim)  # GroupNorm(1) = InstanceNorm, works with batch_size=1
        self.lin3 = ConditionalConv2d(feature_dim, feature_dim, n_steps)
        self.unetnorm3 = nn.GroupNorm(1, feature_dim)  # GroupNorm(1) = InstanceNorm, works with batch_size=1
        self.lin4 = nn.Conv2d(feature_dim, y_dim, kernel_size=1, stride=1)
        self.cond_weight = nn.Parameter(torch.randn((1, feature_dim, 7)), requires_grad=True)
...
class ResNetEncoder(nn.Module):
    def __init__(self, arch='resnet18', feature_dim=128, config=None, local=False):
        ...
        if arch == 'resnet50':
            ...
            # Use timm's EfficientNet-B0 as a backbone (no classifier, global pooled)
            backbone = create_model(
                'efficientnet_b0',
                pretrained=True,
                num_classes=0,           # removes classifier, returns features
                global_pool='avg'        # global average pooling to vector
            )
            self.backbone = backbone
            self.featdim = backbone.num_features
            self._timm_backbone = True
        elif arch == 'vit':
    ...
    def forward_feature(self, x):
        if self._timm_backbone:
            feature = self.backbone(x)
            # timm may return (B, C) or (B, C, H, W) depending on settings; ensure vector
            if feature.dim() == 4:
...
class SamEncoder(nn.Module):
    def __init__(self, arch='resnet18', feature_dim=128, config=None, image_size=512):
        super(SamEncoder, self).__init__()

        self.f = []
        efficient_sam_model = build_efficient_sam_vits(img_size=image_size)
        self.f = efficient_sam_model.image_encoder
        self.featdim = self.f.transformer_output_dim
