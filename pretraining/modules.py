...
class DownsampleNetworkEfficientNetB0(nn.Module):
    """
    Downsampling using EfficientNet-B0
    Better feature extraction for medical images
    Output: 1280 channels (vs ResNet18's 512)
    """
    def __init__(self):
        super(DownsampleNetworkEfficientNetB0, self).__init__()
...
class GlobalNetwork(AbstractMILUnit):
    """
    Implementation of Global Network using EfficientNet-B0
    """
    def __init__(self, parameters, parent_module):
        super(GlobalNetwork, self).__init__(parameters, parent_module)
        # downsampling-branch using EfficientNet-B0
        if "use_v1_global" in parameters and parameters["use_v1_global"]:
            self.downsampling_branch = DownsampleNetworkEfficientNetB0()
        else:
...
class LocalNetwork(AbstractMILUnit):
    ...
    def add_layers(self):
        ...
        :return:
        """
        self.parent_module.dn_efficientnet = DownsampleNetworkEfficientNetB0()
    ...
    def forward(self, x_crop):
        ...
        :return:
        """
        # forward propagate using EfficientNet-B0
        res = self.parent_module.dn_efficientnet(x_crop.expand(-1, 3, -1 , -1))
...
class AttentionModule(AbstractMILUnit):
    ...
    Updated for EfficientNet-B0 (1280 dims)
    """
    def add_layers(self):
        """
        Function that add layers to the parent module that implements nn.Module
        :return:
        """
        # The gated attention mechanism (1280 -> 128 for EfficientNet-B0)
        self.parent_module.mil_attn_V = nn.Linear(1280, 128, bias=False)
        self.parent_module.mil_attn_U = nn.Linear(1280, 128, bias=False)
        self.parent_module.mil_attn_w = nn.Linear(128, 1, bias=False)
        # classifier (1280 -> num_classes for EfficientNet-B0)
        self.parent_module.classifier_linear = nn.Linear(1280, self.parameters["num_classes"], bias=False)
